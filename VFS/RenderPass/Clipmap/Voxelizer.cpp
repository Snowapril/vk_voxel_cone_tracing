...
namespace vfs
{
	Voxelizer::Voxelizer(CommandPoolPtr cmdPool, uint32_t voxelResolution)
	...
	Voxelizer::Voxelizer(CommandPoolPtr cmdPool, uint32_t voxelResolution,
		...
		: RenderPassBase(cmdPool)
	{
		assert(initializeVoxelizer(voxelResolution, framebufferExtent));
	}
	...
	bool Voxelizer::initializeVoxelizer(uint32_t voxelResolution, VkExtent2D framebufferExtent)
	{
		_voxelResolution = voxelResolution;
		
		createAttachments(framebufferExtent);
		createRenderPass();
		createFramebuffer(framebufferExtent);
		createVoxelClipmap();
		createDescriptors();
		return true;
	}
	...
	{
		// TODO(snowapril) : use VK_imageless_framebuffer. this pass do not need render target attachment
		// https://github.com/KhronosGroup/Vulkan-Guide/blob/master/chapters/extensions/VK_KHR_imageless_framebuffer.adoc
		...
		imageInfo.samples		= getMaximumSampleCounts(_device);
	...
	Voxelizer& Voxelizer::createRenderPass(void)
	{
		...
		depthDesc.samples		 = getMaximumSampleCounts(_device);
		...
		assert(_renderPass->initialize(_device, { depthDesc }, {}, { subpassDesc }));
		return *this;
	}
	...
	Voxelizer& Voxelizer::createVoxelClipmap(void)
	{
	...
	void Voxelizer::cmdVoxelize(VkCommandBuffer cmdBufferHandle, const ClipmapRegion& region, uint32_t clipLevel)
	{
		...
		vxDesc.regionMinCorner = glm::vec3(region.minCorner) * region.voxelSize - glm::vec3(1e-6f);
		vxDesc.regionMaxCorner = glm::vec3(region.minCorner + glm::ivec3(region.extent)) * region.voxelSize + glm::vec3(1e-6f);
	...
	void Voxelizer::setViewProjection(VkCommandBuffer cmdBufferHandle, const ClipmapRegion& region, uint32_t clipLevel)
	{
		...
		glm::mat4 viewProj[6];
		viewProj[0] = glm::ortho(-regionGlobal.z, regionGlobal.z, -regionGlobal.y, regionGlobal.y, 0.1f, regionGlobal.x) *
					  glm::lookAt(eye, eye + glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		viewProj[1] = glm::ortho(-regionGlobal.x, regionGlobal.x, -regionGlobal.z, regionGlobal.z, 0.1f, regionGlobal.y) *
				      glm::lookAt(eye, eye + glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(0.0f, 0.0f, -1.0f));
		viewProj[2] = glm::ortho(-regionGlobal.x, regionGlobal.x, -regionGlobal.y, regionGlobal.y, 0.1f, regionGlobal.z) *
	...
	void Voxelizer::processWindowResize(int width, int height)
	{
		...
		VkExtent2D newResolution = { static_cast<uint32_t>(width), static_cast<uint32_t>(height) };
		// vkDeviceWaitIdle(_device->getDeviceHandle());

		// createAttachments(newResolution);
		// createRenderPass();
