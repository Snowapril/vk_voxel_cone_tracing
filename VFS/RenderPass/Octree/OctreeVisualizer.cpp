...
namespace vfs
{
	OctreeVisualizer::OctreeVisualizer(DevicePtr device, CameraPtr camera, 
									   std::shared_ptr<OctreeBuilder> builder, VkRenderPass renderPass)
	{
	...
	bool OctreeVisualizer::initialize(DevicePtr device, CameraPtr camera, 
									  std::shared_ptr<OctreeBuilder> builder, VkRenderPass renderPass)
	{
		_device			= device;
		_camera			= camera;
	...
	void OctreeVisualizer::buildPointClouds(const CommandPoolPtr& cmdPool)
	{
		...
		constexpr uint32_t kVisualizationTargetLevelMax = 2;
		constexpr uint32_t kVisualizationTargetLevelMin = 2;
				...
				*/
				...
				for (uint32_t i = 0; i < 8; ++i)
				{
					if ((baseIndex < nodeData.size()) && ((nodeData[baseIndex + i].x & 0x80000000) != 0))
					{
						...
						const uint32_t newColor = nodeData[baseIndex + i].y;
	...
	bool OctreeVisualizer::initializePipelines(VkRenderPass renderPass)
	{
		...
		_avcPipeline->attachShaderModule(VK_SHADER_STAGE_VERTEX_BIT,   "Shaders/avc.vert.spv", nullptr);
		_avcPipeline->attachShaderModule(VK_SHADER_STAGE_GEOMETRY_BIT, "Shaders/avc.geom.spv", nullptr);
		_avcPipeline->attachShaderModule(VK_SHADER_STAGE_FRAGMENT_BIT, "Shaders/avc.frag.spv", nullptr);
